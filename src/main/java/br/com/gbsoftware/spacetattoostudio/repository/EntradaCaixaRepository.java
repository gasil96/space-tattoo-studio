...
public interface EntradaCaixaRepository extends JpaRepository<EntradaCaixa, Long> {
	...
	List<EntradaCaixa> findByEntradaSaida(@Param("iDCaixaFK") Long iDCaixaFK);
	
	@Query(value = "select *  \r\n" + 
			"from entrada_saida where  date_format(horario_operacao, '%d-%m-%Y') between :dataInicial and :dataFinal", nativeQuery =true )
	List<EntradaCaixa> findByInterval(@Param("dataInicial")String dataInicial, @Param("dataFinal") String dataFinal);
